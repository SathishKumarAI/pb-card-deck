...
- **This repo is the label `repo:pb-card-deck`.** Every work item you create must carry it.
...
1. **Before starting work, check for an existing work item** for what you are
   about to do. Search the board by `repo:pb-card-deck` first. Duplicates are worse
...
2. **A found bug outside the current task's scope gets filed, not silently left.**
   File it in `Backlog` with `repo:pb-card-deck`, say in your reply that you filed it.
